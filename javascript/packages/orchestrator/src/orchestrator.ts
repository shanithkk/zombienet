...
export async function start(
  ...
  if (launchConfig.settings.provider == "kurtosis") {
    ...
    try {
      ...
      const zombieWrapperContent =
        await fs.promises.readFile(zombieWrapperPath);
      ...
      const promiseGenerators = networkSpec.relaychain.nodes.map(
        (node: Node) => {
          return () =>
            spawnNode(
              client,
      ...
      debug(
        `\t 🚀 LAUNCH COMPLETE under namespace ${decorators.green(
          namespace,
        )} 🚀`,
      );
